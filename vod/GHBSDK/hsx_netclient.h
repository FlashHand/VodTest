...
typedef enum eLogQuerySubType
{
	...
	eLogAlarmAlarmInOver,               // 报警输入结束子类型
	eLogAlarmEnd,
	...
	eLogExcpetionAddIpc ,               //自动添加ipc
	eLogExceptionRemoveIpc ,		   //自动删除ipc
	eLogExceptionEnd,
...
typedef struct tagDiscoveryInfo
{
	...
	char    ip_address[32];             //ip
    
    char    reserve1[96];
    
	int     port;
	int     device_type;				// 设备类型(参考定义gk_device_type)
	int     ipc_type;					// IPC协议类型(参考hsx_cfgdefine.h中的定义eIPCType)
	char    wifi_ipaddr[32];			// wifi无线ip地址
	char    username[32];				// 设备的默认用户名
	char    password[32];				// 设备的默认密码
...
typedef int (CALLBACK *CB_PlaybackData)(HLIVE hStream, frame_header_t *pFrameHeader, BYTE *pDataBuf, DWORD dwDataLen, DWORD dwUserData);
...
// 设备发现回调
typedef int(CALLBACK* CB_Discovery)(HDISCOVERY hDiscovery, DiscoveryInfo *pDiscoveryInfo, int nIsNewDevice, long long dwUserData);
...
// 启动实时流
DVR_NET_API int __stdcall Network_ClientStartLive(HLIVE *pStream, IN HDEVICE hDev, LiveConnect *pClientInfo, CB_StreamData cDatacbf, DWORD dwUserData);
...
// bNeedSound, 回调是否需要声音
DVR_NET_API int __stdcall Network_ClientSetLiveDataCB(HLIVE hStream, CB_StreamData cDatacbf, DWORD dwUserData, bool bMustCBMainStream, bool bNeedSound);
...
DVR_NET_API int __stdcall Network_ClientStartPlaybackEx(OUT HPLAYBACK *pPlayback, HANDLE hDev, DWORD dwChannel, DWORD dwFileType,
                                                        TimeInfo *pBeginTime, TimeInfo *pEndTime, CB_PlaybackData cDatacbf);
...
// 发送语音数据
DVR_NET_API int __stdcall Network_ClientSendtVoiceData(OUT HVOICE hVoice, IN BYTE *data , IN int nDataLen);
// 停止语音对讲
...
//adpcm codec
DVR_NET_API long long __stdcall Network_AdpcmOpen();
DVR_NET_API void __stdcall Network_AdpcmClose(long long);
DVR_NET_API int __stdcall Network_AdpcmEncode(long long handle, short *src, int cap_num, int src_stride, char *dst, int dst_size);
//DVR_NET_API long long __stdcall Network_AdpcmDecode();
